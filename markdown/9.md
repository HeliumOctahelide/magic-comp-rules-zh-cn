...
<b id='cr901-4'>901.4.</b> 整个游戏过程当中，所有的时空牌和异象牌均位于统帅区中，无论是在它们仍属时空套牌的部分，或是处于牌面朝上期间，均是如此。   
...
<b id='cr901-10'>901.10.</b> 当一位牌手离开游戏时，由该牌手拥有的所有物件均会离开游戏，但异象产生的异能除外。（参见规则[800.4a](/cr/8/#cr800-4a)。）如果离开游戏的物件中包括牌面朝上的时空牌或异象牌，则时空操控者将其时空套牌牌库顶牌翻回正面。这不属于状态动作。这会在牌手离开游戏的同时发生。   
...
<b id='cr901-12b'>901.12b</b> 通常情况下，时空操控者为主动队伍的主要牌手。然而，如果当前时空操控者所属队伍将离开游戏，则改为依照回合顺序下一支不会离开游戏之队伍的主要牌手成为时空操控者，然后原先的时空操控者队伍离开游戏。新的时空操控者保持该称号直到发生下述两种情况之一为止：该牌手离开游戏；或有其他队伍成为主动队伍。   
...
<b id='cr901-13'>901.13.</b> 除了大型混战之外，在其他的多人游戏玩法中，时空牌和异象牌均不受限制影响范围模式规则限制。它们的异能及该些异能的效应，会对游戏中所有适用的物件和牌手产生影响。（参见规则[801](/cr/8/#cr801)，“限制影响范围模式”。）   
...
<b id='cr901-15a'>901.15a</b> 竞逐时空游戏也能以使用单一共用时空套牌的玩法进行游戏。在此情况下，时空套牌须至少为四十张或至少十倍于参加游戏的牌手数量，取两者间较小者。时空套牌中包含的异象牌数量，不得多于参加游戏之牌手数量的两倍。时空套牌中各牌的英文名称须各不相同。   
...
<b id='cr901-15c'>901.15c</b> 如果有任何规则或异能提及某牌手的时空套牌，则使用共用的时空套牌。   
...
<b id='cr902-4'>902.4.</b> 每位牌手的起始总生命为20加上或减去其先锋牌上的生命修正值。   
...
<b id='cr902-5'>902.5.</b> 每位牌手的起手牌张数为七张，且会受其先锋牌之手牌修正值影响。   
<b>902.5.</b> Each player’s starting hand size is seven cards, as modified by the hand modifier of their vanguard card.

<b id='cr902-5a'>902.5a</b> 如果某牌手在先锋游戏中进行再调度，则该牌手如在正常游戏中进行再调度一般，将其手牌洗回其牌库，然后抓新的手牌，其数量等同于其起手牌张数。（在多人游戏中，牌手第一次进行再调度后所抓的手牌数与之前相同。）参见规则[103.5](/cr/1/#cr103-5)。   
...
<b id='cr903-7'>903.7.</b> 在确定先手牌手之后，每位牌手将总生命设为40，并抓一份七张牌的手牌。   
...
<b id='cr903-12g'>903.12g</b> 在任何一种争锋游戏中，牌手第一次执行的再调度不会计入其应置于其牌库底的牌张数量，也不会计入其可以执行再调度的次数。第一次再调度之后的再调度会正常计入该数量。   
...
<b id='cr903-13e'>903.13e</b> 在轮抽结束后，每位牌手所抽选的牌成为该牌手的牌池。如果该次轮抽包含指挥官传奇系列或指挥官大师系列的轮抽补充包，则牌手可以向其牌池至多添加两张名称为虹彩魔笛手的牌，但仅限于这些牌被用作该牌手指挥官的情形下。如果该次轮抽包含指挥官传奇：争战博德之门®系列的轮抽补充包，则每位牌手可以向其牌池添加至多两张名称为无面者的牌，但仅限于这些牌被用作该牌手指挥官的情形下。   
...
<b id='cr903-13g'>903.13g</b> 指挥官轮抽游戏遵循与指挥官游戏一样的规则。参见规则[903.6-903](/cr/9/#cr903-6)11。   
...
<b id='cr904-4'>904.4.</b> 整个游戏过程当中，所有邪计牌均位于统帅区中，无论是在它们仍属邪计套牌部分，或是处于牌面朝上期间，均是如此。   
...
<b id='cr904-5'>904.5.</b> 魔王的起始总生命为40。其他牌手的起始总生命为20。   
...
<b id='cr904-12c'>904.12c</b> 如正常多人自由竞赛游戏一般，采用随机方式决定先手牌手。魔王游戏中所有作用于魔王的其他规则，在群魔乱斗游戏中则会适用于所有牌手。   
...
<b id='cr905-3'>905.3.</b> 诡局轮抽游戏是多人游戏。默认使用的多人模式是自由竞赛模式，使用攻击复数牌手模式，并且不使用限制影响范围模式。参见规则[806](/cr/8/#cr806)，“自由竞赛模式”。   
