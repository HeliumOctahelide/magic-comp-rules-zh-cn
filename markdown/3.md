...
<b id='cr301-5e'>301.5e</b> 如果某效应试图将一个非灵气的武具（参见规则[303.4i](/cr/3/#cr303-4i)）放进战场并贴附在一个不能合法装备或未定义的物件上，该武具进战场时未贴附。如果该武具为衍生物，则将其派出且进战场时未贴附。   
...
<b id='cr303-4c'>303.4c</b> 如果灵气结附在其结附异能或其他适用效应所定义的非法物件或牌手上，或是所贴附的物件不再存在，或所贴附的牌手已经离开游戏，则此灵气置入其拥有者的坟墓场。（此为状态动作。参见规则[704](/cr/7/#cr704)。）   
...
<b id='cr303-4e'>303.4e</b> 灵气的操控者与所结附的物件的操控者或所结附的牌手彼此独立；他们并不需要是同一人。如果灵气结附于物件上，改变该物件的操控权并不会改变灵气的操控权，反之亦然。只有灵气的操控者才能起动它的异能。如果灵气赋予所结附的物件某个异能（使用“获得”或“具有”），则只有所结附的物件之操控者才能起动该异能。   
...
<b id='cr303-4f'>303.4f</b> 如果灵气并非以结算灵气咒语的方式，而是以其他方式在一位牌手的操控下进战场，并且将它放置进战场的效应并未指定它将结附在哪里，则于进战场时，此灵气在其操控下进战场的牌手指定将它结附在哪个物件或牌手上。该牌手必须依据该灵气的结附异能和其他效应，来选择可合法结附的物件或牌手。   
...
<b id='cr305-2a'>305.2a</b> 确定牌手是否能使用一个地的方法是，对比该牌手本回合可以使用地的数量和其本回合已经使用过地的数量（包括作为特殊动作使用的地和咒语和异能结算过程中使用的地）。如果牌手可以使用地的数量更大，则该使用合法。   
...
<b id='cr305-3'>305.3.</b> 牌手不能因为任何原因在不是自己的回合中使用地。忽略任何效应中指示牌手如此作的部分。   
...
<b id='cr305-7'>305.7.</b> 如果一个效应将地的类别改为一种或数种基本地类别，该地便不再拥有原本之地类别。它失去所有原本的规则叙述、地类别以及任何影响该地的可复制效应所带来的异能，且获得新的基本地类别所对应的法术力异能。由其他效应赋予此地的异能并不会因此移除。改变地的副类别不会添加或移除该地可能拥有的牌张类别（例如生物）或超类别（例如基本、传奇、雪境）。如果地额外得到原有地类别以外的其他地类别，则它会保留原本的地类别与规则叙述，且得到新的地类别及法术力异能。   
...
<b id='cr307-5a'>307.5a</b> 同样的，如果一个效应检查一个咒语是否“于不能施放法术的时机下”被施放，它仅检查该咒语的操控者是否在不具有优先权时、在自己回合的行动阶段以外，或在堆叠上有其他物件时施放该咒语。   
...
<b id='cr309-5a'>309.5a</b> 如果牌手在统帅区拥有一张地城牌、且其进度标记不在该地城的最底下的房间上时深入地城，该牌手按照某个从进度标记所在的房间出发的箭头指示，将进度标记从当前的房间移动到下一个房间上。如果从该牌手的进度标记所在的房间出发的箭头有多个，该牌手选择其中一个指示来移动。   
...
<b id='cr310-9a'>310.9a</b> 于战役进战场时，其操控者选择一位牌手来作为其防卫者。哪些牌手能够被选择为防卫者取决于其战役类别（参见规则[310.12](/cr/3/#cr310-12)）。如果它没有战役类别，只有其操控者可以是其防卫者。   
...
<b id='cr310-9d'>310.9d</b> 如果战役的防卫者并非其操控者，则与正在被攻击的战役相关的所有提及“防御牌手”的规则和效应均指该战役的防卫者，而非其操控者。参见规则[508.5](/cr/5/#cr508-5)。   
...
<b id='cr310-9e'>310.9e</b> 如果某规则或效应提及防卫某战役的牌手，则其所指的是为该战役防卫者的牌手。   
...
<b id='cr310-12a'>310.12a</b> 于围攻进战场时，其操控者须选择一位对手来作为其防卫者。只有围攻的操控者之对手可以作为其防卫者。   
...
<b id='cr311-7'>311.7.</b> 时空牌具有“每当引发混沌”时触发的触发式异能。这些异能称为“混沌异能”。这些异能的左边有{CHAOS}符号用以指示，即使该符号没有特别规则含义。此异能在以下情况下触发：如果时空骰的掷骰结果为混沌符号（参见规则[901.9b](/cr/9/#cr901-9b)）；如果一个结算中的咒语或异能叙述引发混沌；或如果一个结算中的咒语或异能叙述一个特定物件引发混沌。在最后一种情况下，即使该时空牌仍在牌库中但被展示，混沌异能也能够触发。混沌异能由当前时空的操控者所操控。   
...
<b id='cr313-6'>313.6.</b> 每张先锋牌的左下角都印有手牌修正。手牌修正可能是带有加号的数字、带有减号的数字、或零。确定先锋牌拥有者的起手牌张数和手牌上限（通常为七张）时，采用此修正值。修正后的数字便是该牌手在游戏开始时所抓牌的数量以及手牌上限。参见规则[103.5](/cr/1/#cr103-5)。   
...
<b id='cr314-4'>314.4.</b> 邪计牌可以具有任意数量的静止式、触发式和／或起动式异能。只要邪计牌在统帅区中为牌面朝上，其静止式异能便影响游戏，触发式异能便可以触发，起动式异能可以起动。   
