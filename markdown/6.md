...
例如：伊捷守护者梅列克的部分叙述为“你可以从你的牌库顶施放瞬间和法术咒语。”如果你操控伊捷守护者梅列克，且你的牌库顶牌是巨人杀手（一张历险名称为连根砍倒的瞬间之历险者生物牌），你可以施放连根砍倒，但不能施放巨人杀手。倘若此时你操控的不是梅列克、而是贾路的兽群，则你可以从牌库顶施放巨人杀手，而不能施放连根砍倒。   
...
<b id='cr603-2d'>603.2d</b> 异能可能会叙述一个触发式异能额外触发数次。在此情形下，应决定该异能应当触发多少次（而非仅仅决定其已触发），然后该异能触发等量的次数。叙述一个异能额外触发数次的效应不会反复引发本身，且不会适用于影响异能触发次数的其他效应。叙述某物件的触发式异能额外触发数次的效应，仅指该物件具有的触发式异能，不包括该物件具有的异能可能会产生的任何延迟触发式异能或自身触发式异能（参见规则[603.7](/cr/6/#cr603-7)和规则[603.12](/cr/6/#cr603-12)）。   
...
例如：一个当永久物“成为横置”而触发的异能，只有在一个已经存在于战场上的永久物的状态从未横置成为横置时才会触发。   
...
<b id='cr603-3'>603.3.</b> 一旦异能触发，其操控者于下一次有牌手将得到优先权时将其作为非牌物件放进堆叠。参见规则[117](/cr/1/#cr117)，“时机和优先权”。该异能成为堆叠最顶端的物件。它具有创造它的异能所具有的文字叙述，且不具有任何其他特征。该异能直到被反击、结算、被规则移出堆叠或一个效应将其移到其他地方之前将一直保持留在堆叠上。   
...
<b id='cr603-4'>603.4.</b> 一个触发式异能的格式可能为“当／每当／在[触发事件]，若[条件]，[效应]”。当该触发事件发生时，该异能检查该条件是否符合。该异能只有在条件符合时才会触发；否则它将没有效果。如果该异能触发，则于其结算时再次检查该条件是否符合。如果该条件此时不再符合，该异能从堆叠中被移除且没有任何效果。注意这将与检查合法目标类似地重复进行检查。此规则被称为“以‘若’开头的子句”。（“若”一词在牌其他部分的文字叙述中出现则只具有它的一般中文意义；此规则只对在触发条件之后紧接的“若”生效。）   
...
<b id='cr603-9'>603.9.</b> 一些触发式异能特指当一位牌手输掉游戏时触发。这些异能当有牌手输掉或离开游戏时触发，无论原因为何，除非该牌手由于平手离开游戏。参见规则[104.3](/cr/1/#cr104-3)。   
...
<b id='cr603-10a'>603.10a</b> 一些改变区域触发需要回溯。此类异能包括离开战场异能、当牌手牺牲一个永久物时触发的异能、当一张牌离开坟墓场时触发的异能、及当一个所有牌手可见的物件置入手牌或牌库中时触发的异能。   
...
<b id='cr603-10f'>603.10f</b> 当一位牌手输掉游戏时触发的异能需要回溯。   
...
<b id='cr608-2b'>608.2b</b> 如果该咒语或异能指定了目标，它检查这些目标是否依然合法。一个不再存在于之前宣告为目标时所在区域的目标不合法。其他对游戏状态的更改可能会导致目标不再合法；例如，其特征可能被改变或一个效应可能改变了咒语的叙述。如果异能之来源离开了其原本所在的区域，则在此过程中会使用该来源的最后已知信息。如果一个咒语或异能的每个“目标”一词对应的目标均非法，该咒语或异能不会结算；将其从堆叠上移除，且如果它是咒语，将其置入其拥有者的坟墓场。否则，它将正常结算。非法目标（如果有的话）不会受到该咒语或异能的效应中关于这些非法目标部分之影响。如果它们对该效应的其他部分而言仍是合法目标，则可能受到该部分的影响。如果该咒语或异能创造了一个影响游戏规则的持续性效应（参见规则[613.11](/cr/6/#cr613-11)），该效应不对非法目标生效。如果该效应的一部分需要确定非法目标的信息，该效应无法确定任何信息。该效应中任何需要这些信息以生效的部分不会生效。   
...
例如：疫病孢子为“消灭目标非黑色生物和目标地。它们不能重生。”假设同一个生物地作为非黑色生物和地被宣告为目标，且在疫病孢子结算前它的颜色被改为黑色。疫病孢子仍然会结算，因为该黑色生物地依然是该咒语“目标地”部分的合法目标。该咒语“消灭目标非黑色生物”的部分不会影响该永久物，但“消灭目标地”的部分仍会将其消灭。它不能重生。   
...
<b id='cr608-2k'>608.2k</b> 如果一个异能的效应引用一个特定且未成为目标的物件，且该物件之前被该异能的费用或触发条件引用过，则它依然会影响该物件，即使它的特征已经改变。   
...
<b id='cr608-2p'>608.2p</b> 一旦完成608.2c-n中所述之所有可能的步骤，任何在该咒语或异能结算时触发的异能触发。   
...
<b id='cr608-3c'>608.3c</b> 如果正在结算的物件是灵气咒语，则该咒语成为永久物，在其操控者的操控下被放进战场，并贴附于其目标所指的牌手或物件上。   
...
<b id='cr609-4'>609.4.</b> 一些效应叙述牌手可以“视同”某些条件正确的情况下作一些事，或令生物“视同”某些条件正确的情况下作一些事。这只对该叙述的效应生效。对于该效应来说，将游戏当作该条件为正确来对待。对于其他情况，将游戏按照一般情况对待。   
...
<b id='cr609-4a'>609.4a</b> 如果两个效应叙述牌手可以（或生物可以）“视同”两个不同的条件正确的情况下作一些事，这两个条件可能都会生效。如果其中一个被“视同”的效应使另一个被“视同”的效应之条件得到满足，则两者均生效。   
...
<b id='cr611-2a'>611.2a</b> 一个由咒语或异能的结算所产生的持续性效应，在产生它的咒语或异能所注明的时段内生效（例如“直到回合结束”）。如果没有注明时段，则它生效直到游戏结束。   
...
<b id='cr612-6'>612.6.</b> 一张牌（瓦拉司的变形兽）叙述一个物件具有另一个物件的“全部叙述”。这不仅仅改变该物件文字栏和类别栏中的叙述，同样改变表示其名称、法术力费用、颜色标志、力量以及防御力的叙述。   
...
<b id='cr612-7'>612.7.</b> 一张牌（谍报用具）叙述一个物件具有“所有非传奇生物牌之名称”。这会改变代表该物件之名称的叙述。该物件具有Oracle牌张参考文献中的每张非传奇生物牌之名称。（参见规则[108.1](/cr/1/#cr108-1)。）   
...
<b id='cr613-3'>613.3.</b> 在层2-6中，特征定义异能所产生的效应首先生效（参见规则[604.3](/cr/6/#cr604-3)），然后所有其他效应按照时间印记顺序生效（参见规则[613.7](/cr/6/#cr613-7)）。注意，从属关系可能会改变同一层中效应生效的顺序。（参见规则[613.8](/cr/6/#cr613-8)。）   
...
<b id='cr613-8a'>613.8a</b> 如果一个效应（a）与另一个效应在同一层（或副层）中生效；（b）第二个效应生效与否，将改变第一个效应的叙述或存在、改变它将生效的对象、或改变它将生效的内容；以及（c）这两个效应都是或都不是特征定义异能，则第一个效应“从属于”第二个效应。否则，第一个效应不从属于第二个效应。   
...
例如：两个结附于同一个生物上的灵气：其中一个为“所结附的生物具有飞行异能”而另一个为“所结附的生物失去飞行异能”。两者不从属于彼此，因为没有改变受它们影响的对象或它们影响的内容。令它们按照时间印记顺序生效表示最后产生的一个“获胜”。其中的效应是否是暂时的（例如“目标生物失去飞行异能直到回合结束”）或为非灵气来源的（例如“所有生物失去飞行异能”）并不会影响生效流程和最终结果。   
...
<b id='cr614-11a'>614.11a</b> 如果一个效应替代一系列抓牌其中的一次抓牌，则先尽可能完成替代所要求的所有动作，再继续此系列抓牌。   
...
<b id='cr614-14'>614.14.</b> 一个物件可能印有一个产生替代性效应的异能，该异能将导致一张或数张牌被放逐，以及另一个异能提及“被放逐的牌”或“被[此物件]放逐”的牌。这些异能互相关联：第二个异能提及的牌只包括放逐区中被第一个异能所产生的替代事件直接放逐的牌。如果另一个物件得到一对关联异能，这些异能在该物件上会以类似的形式互相关联。它们不能与任何其他异能有所关联，无论该物件当前或之前可能具有的其他异能。参见规则[607](/cr/6/#cr607)，“关联异能”。   
...
<b id='cr615-7'>615.7.</b> 一些由咒语或异能的结算而产生的防止性效应提及特定数量的伤害；例如，“防止本回合中接下来将对任意一个目标造成的3点伤害。”它们如护盾一般。将对该具有“护盾”的永久物或牌手造成的每1点伤害均将被防止。每防止1点伤害则剩余的护盾减少1。如果有两个或以上适用的来源将对具有护盾的永久物或牌手同时造成伤害，该牌手或该永久物的操控者选择该护盾将防止哪些伤害。一旦护盾降至0，任何剩余伤害将照常造成。此类效应只计算伤害的数量；造成伤害的事件或来源数量无关紧要。   
